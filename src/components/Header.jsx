    ...
    <div className="flex justify-between items-center mb-1">
      ...
      <Link to="/" className="flex flex-col items-center group">
        <motion.img 
          ...
          className="md:h-20 h-14 ml-1 transition-transform duration-300 group-hover:scale-105" 
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        />
        <p className="text-xs md:text-sm ml-2 font-semibold group-hover:text-green-700 transition-colors duration-300">
          <i>we build trust</i>
        </p>
      </Link>
          ...
          <React.Fragment key={link.name}>
            <motion.div whileHover={{ y: -2 }} whileTap={{ y: 0 }}>
              <NavLink
                to={link.to}
                className={({ isActive }) =>
                  `px-3 py-2 text-black text-lg lg:text-xl font-sans font-semibold 
                   rounded-lg border-2 border-transparent  
                   transition-all duration-300 ease-in-out 
                   hover:border-green-600 hover:shadow-lg hover:bg-green-50 
                   ${isActive ? "border-green-600 shadow-lg bg-green-100" : ""}`
                }
              >
                {link.name}
              </NavLink>
            ...
            {link.name === "About" && (
              ...
              >
                <motion.button
                  className="px-3 py-2 text-black text-lg lg:text-xl font-sans font-semibold 
                             rounded-lg border-2 border-transparent /* ✅ Fix alignment */
                             transition-all duration-300 ease-in-out 
                             hover:border-green-600 hover:shadow-lg hover:bg-green-50 flex items-center gap-1"
                  whileHover={{ y: -2 }}
                  whileTap={{ y: 0 }}
                >
                  Products
                ...
                <AnimatePresence>
                  {isProductsOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      transition={{ duration: 0.2 }}
                      className="absolute top-full left-0 mt-2 w-56 md:w-64 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-50"
                    >
          ...
          <ul className="flex flex-col items-center space-y-2">
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block px-4 py-2 text-black font-semibold font-sans rounded 
                       ...
                       ${isActive ? "bg-green-700 text-white border-green-600" : "hover:bg-green-700 hover:border-green-600"}`
                    }
            ...
            <li className="w-full">
              <motion.button
                ...
                className="w-full px-4 py-2 text-black font-semibold font-sans rounded 
                           border-2 border-transparent hover:border-green-600 hover:bg-green-700 transition-all duration-300 flex items-center justify-center gap-1"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                Products
